...
def train_one_epoch(args, train_loader, model, criterion, optimizer, epoch, device):
    ...
    train_trues = []
    train_scores = []
    for idx, (_, peptides, alleles,labels, peptide_graphs, pseudo_graphs) in enumerate(train_loader):
...
def valid(valid_loader, model, criterion, epoch, device):
    ...
    with torch.no_grad():
        model.eval()
        for idx, (_, peptides, alleles,labels, peptide_graphs, pseudo_graphs) in enumerate(valid_loader):
            peptide_graphs = Batch.from_data_list(peptide_graphs)
            peptide_graphs = peptide_graphs.to(device)
            pseudo_graphs = Batch.from_data_list(pseudo_graphs)
            pseudo_graphs = pseudo_graphs.to(device)
...
def test(test_loader, model, device):
    ...
    all_alleles = []
    with torch.no_grad():
        for idx, (_, peptides, alleles,labels, peptide_graphs, pseudo_graphs) in enumerate(test_loader):
            peptide_graphs = Batch.from_data_list(peptide_graphs)
            peptide_graphs = peptide_graphs.to(device)
            pseudo_graphs = Batch.from_data_list(pseudo_graphs)
            pseudo_graphs = pseudo_graphs.to(device)
    ...
    return all_peptides, all_alleles, all_scores, all_trues
...
def Inference(test_file_path, model_path='', multi_process=8):
    ...
    model_args = state['opt']
    args['k'] = model_args.k
    args['hidden_size'] = model_args.hidden_size
    args['depth'] = model_args.depth
    args['heads'] = model_args.heads
    dataset = pMHC_DataSet(test_file_path,num_process=multi_process)
    ...
    all_peptides, all_alleles, all_scores, all_labels = test(dataloader, model, device)

    if 'label' in headline.columns:
        df = pd.DataFrame({"peptide":all_peptides,"HLA":all_alleles,"score":all_scores,"label":all_labels},index=list(range(len(all_peptides))))
    else:
        df = pd.DataFrame({"peptide":all_peptides,"HLA":all_alleles,"score":all_scores},index=list(range(len(all_peptides))))
    if not os.path.exists(args['output']):
        os.makedirs(args['output'])
    df.to_csv(args['output']+'pHLAI_predictions.csv',index=False)
    print('Prediction results have been saved to'+args['output']+'pHLAI_predictions.csv')
    return df
...
def Train(k_fold_dir, config_path=''):
    ...
    train_dataset = pMHC_DataSet(train_path,num_process=int(args['num_process']),aug=True,test=False)
    val_dataset = pMHC_DataSet(val_path,num_process=int(args['num_process']),aug=False,test=False)
